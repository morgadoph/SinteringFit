function [beta,r,J,Sigma,mse, exitf] = nlinfit2(X,y,model,beta,options)
...
if strcmp(options.Robust,'off')
    ...
    [beta,J,lsiter,cause, exitf] = LMfit(X,y, model,beta,options,verbose,maxiter);
else
...
%----------------------------------------------------------------------
function  [beta,J,iter,cause, exitf] = LMfit(X,y, model,beta,options,verbose,maxiter) 
...
aux=isreal(v);
if any(~isfinite(v)) || aux==0
    errordlg('Fitting not sucessful','Warning','modal');
    return;
end


%--------------------------------------------------------------------------
function exitf =  parValCheck (vetor)
exitf=0;
%Check parameters values    
    aux=size(vetor);
    for i=1:aux(1,1)
        if isreal(vetor(i,1))==0
            errordlg('Fitting not sucessful');
            exitf=1;
            return;
        end
    end
...
while maxiter>0
    ...
    [beta,J1,lsiter,cause, exitf] = LMfit(x,yw,modelw,beta0,options,0,maxiter); % 6th arg always silences display
