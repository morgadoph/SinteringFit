classdef AbaDataClass<handle
    ...
    methods
        ...
        function Obj = CriaObjetos (Obj)
            ...
            %Fundos
            Obj.Fundo = ViewConstructor.Fundo (Obj.Component);                                                % Fundo para os botões no Vbox Superior
            Obj.FundoTable = uiextras.HBox('Parent', Obj.Component);                                          % Fundo da tabela dividido em 3 HBox
            
            %Botões
            Obj.AddButton = ViewConstructor.Button(Obj.Fundo,'Add',[0.05 0.15 0.15 0.55], {@AddData});        % Botão Add
            
            %Tabela
            ViewConstructor.Fundo(Obj.FundoTable);                                                        % Espaço vacio lateral esquerdo da tabela
            ...
            ViewConstructor.Fundo(Obj.FundoTable);                                                        % Espaço vacio lateral direito da tabela
            Obj.DeleteHtml = ['<html><img src="file:/' pwd '/Imagens/Icons/DeleteSmall.png"/></html>']; %Delete icon add data table
            Obj.Configure;
            
            %%%%%%%%%%%%%Callback do botão que adiciona arquivos%%%%%%%%%%%
                function AddData(hObject, ~, ~)
                    ...
                    %Executa métodos que modificam a aba Strain
                    ...
                    AbaStrain.RefreshPopup(CurveName);
                                            ...
                                            %set(handleAba2.popup(1),'String',CurveName)                %Atualiza string com os nomes das curvas na aba de correção do strain

                                            %Executa métodos que modificam o controle gráfico
                                            %set(GraphicControl.popup(2),'String',CurveName)            %Atualiza string do popup de escolha do gráfico
                end
        
            %-- Mudar cor do ID ou eliminar arquivo de dados experimentais da tabela
            %-- principal. Esses "botões" são ativados quando o campo é selecionado
        ...
        function Obj = Configure (Obj)
            ...
            set( Obj.FundoTable, 'Sizes', [10 -1 10], 'Spacing', 5 ); % Configuração laterais da tabela
            set( Obj.Component, 'Sizes', [70 -1], 'Spacing', 5 ); % Configuração Vbox Botões / Tabela
        end
