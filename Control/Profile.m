classdef Profile <handle
    ...
    methods
        ...
        function obj = Profile(Id, nCurves)
            ...
            obj.ShrinkageDerFlag=0;
            obj.DensityDerFlag=0;
        ...
        function Obj= ApplyShrinkage (Obj)
            for i=1:Obj.nCurves
               ...
               Obj.ShrinkageFlag=1;
               Obj.Config.MassCorrectionDone =0;
               Obj.ArrayData(1).Densities.MassCorFile=[];
            end
        end
        ...
        function Obj= Calculate_Shrinkage (Obj)
                ...
                if T0correction==1
                    for i=1:ncurvas
                        [Acor{i},Ndados{i},l0cor(i)] = T0cor(Arquivo{i},T0,l0(i));
                    end
                else
                    Acor=Arquivo;
                end
                ...
                if Exptermcorrection==0 && Exptermterminal==1
                    for i=1:ncurvas
                        Acor{i}(:,3)=(Acor{i}(:,3)/l0cor(i))+cetterminal{i}(:,1);
                    end
                end
                ...
                if Exptermcorrection==1 && Exptermterminal==0
                    for i=1:ncurvas
                        Acor{i}(:,3)=(Acor{i}(:,3)/l0cor(i))-cet{i}(:,1);
                    end
                end
                ...
                if Exptermcorrection==1 && Exptermterminal==1
                    for i=1:ncurvas
                        Acor{i}(:,3)=(Acor{i}(:,3)/l0cor(i))+cetterminal{i}(:,1)-cet{i}(:,1);     %Calcula a retração do método comum (soma dos termos)
                    end
                    if MetRafael==1
                        A=cell(ncurvas,1);
                        for i=1:ncurvas
